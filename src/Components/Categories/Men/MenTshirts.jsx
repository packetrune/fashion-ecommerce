...
const MenTshirts = (props) => {
  ...
  const MenTshirts = getProductsByType('menT-shirt');  

  const {CurrentUser}=useContext(UsersContext);
  const {AddToCart}=useContext(CartContext);
  const { wishlist, addToWishlist, removeFromWishlist } = useContext(WishlistContext); // Use the WishlistContext
  ...
  return (
    <div>
      <div className="d-flex flex-wrap gap-3 justify-content-center container" style={{marginTop:'7%'}}>
        {MenTshirts?.map((productItem, index) => {
          return (
