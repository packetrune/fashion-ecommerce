...
const Kids = (props) => {
  const { getProductsByCategory } = React.useContext(ProductsContext);
  ...
  // Create a separate state for addedToCart for each product
  const [addedToCart, setAddedToCart] = useState(kidsProducts.map(() => false));
  ...
  return (
    <div>
      <div className="d-flex flex-wrap gap-3 justify-content-center container" style={{marginTop:'7%'}}>
        {kidsProducts?.map((productItem, index) => {
          return (
                  ...
                  {CurrentUser?.role === 'admin' ? (
                    ...
                    <div>
                      ...
                      {wishlist.includes(productItem.id) ? (
                        <button className="btn border-1 rounded-circle ms-2" onClick={() => handleWishlistToggle(productItem.id)}>
